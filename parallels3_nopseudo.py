"""parallels3: same as parallels2 but every catalogue is scored on the SAME
event set (those Horley's map classifies on both sides). This is the table in
the paper (Horley 40.3 %, numeric 18.0 %, family 10.1 %, frequency 0.7 %,
random 0.6 +- 1.1 %).
...
_src = io.open(os.path.join(HERE, "parallels_nopseudo.py"), encoding="utf-8").read()
_src = _src.replace('if __name__ == "__main__":\n    main()', "")
P = {"__file__": os.path.join(HERE, "parallels_nopseudo.py"), "__name__": "par"}
exec(_src, P)
...
def main():
    ...
    # FAIR SET: keep only the events Horley's map can classify on both sides,
    # and score every catalogue on exactly that set. (parallels2.py scored
    # Horley on 139 events and the mechanical rules on 266, which inflated
    # Horley's ratio 2.2x -> 2.9x.)
    hc = P["horley_class"]
    subs = {k: n for k, n in subs.items() if hc(k[0]) is not None and hc(k[1]) is not None}
    print(f"all events {events} -> fair set {sum(subs.values())} ({len(subs)} distinct pairs)")
